...
- `release.yml` stages a package and creates the matching GitHub Release when
...
package archive, submits it to npm's staging area, and creates the GitHub
Release. A version already public on npm or a Release that already exists is a
safe no-op.

Release notes are generated by GitHub. The workflow formats JujuLeaf trigger
tokens such as `@worker` and the npm package name as code before publication so
they cannot mention an unrelated GitHub user or organization.

To reconcile an existing tag with a missing GitHub Release, run the same
workflow manually:
